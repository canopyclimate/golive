...
func TestEvalFieldErrors(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			"{{.MissingField}}",
			(*T)(nil),
			"can't evaluate field MissingField in type *htmltmpl.T",
		},
		{
		...
		},
		{
			...
			"{{.X}}",
			(*T)(nil),
			"nil pointer evaluating *htmltmpl.T.X",
		},
		{
			"MissingKeyOnNilMap",
			"{{.MissingKey}}",
...
func TestExecutePanicDuringCall(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			`template: t:1:23: executing "t" at <.NonEmptyInterfaceNil.Method0>: nil pointer evaluating htmltmpl.I.Method0`,
		},
	}
	for _, tc := range tests {
		b := new(bytes.Buffer)
